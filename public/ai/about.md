# Propel-Lab LLC — Company Overview

## About

Propel-Lab LLC (合同会社 Propel-Lab) is an AI/LLM technology company based in Fukuoka, Japan, founded in April 2025. We engineer next-generation business models through systems thinking and AI technology.

## Mission

"Engineer the next generation of business models through systems thinking and technology."

## Company Information

- **Legal Name**: Propel-Lab LLC (合同会社 Propel-Lab)
- **Location**: Tenjin 4-6-28, Tenjin First Bldg. 7F, Chuo-ku, Fukuoka, Japan
- **Founded**: April 24, 2025
- **Founder & CEO**: Ken Imoto (井本 賢)
- **Email**: info@propel-lab.co.jp
- **Website**: https://propel-lab.co.jp/

## Core Business Areas

1. **LearningMate** — Educational content and publishing focused on LLM technologies (Claude Code, Context Engineering, LLMO)
2. **AI-Loop** — SNS automation and content marketing support powered by LLM agents
3. **AutoCrew** — Business automation consulting using multi-agent LLM systems (MCP, CLI Skills)
4. **KaoriQ** — AI-driven autonomous e-commerce operations (kaoriq.com fragrance platform)
5. **mypcrig** — AI-driven autonomous content media (mypcrig.com PC-selection guides with GA4 + Search Console feedback loop)
6. **legacydram** — AI-driven autonomous curation media (legacydram.com whisky curation through an engineer's lens)
7. **Yureru** — Music and creative project capturing emotional moments through AI-assisted music production

## Technology Expertise

- AI Agent Design (Multi-agent, MCP)
- Context Engineering (CLAUDE.md, prompt design)
- LLMO (LLM Optimization — AI search optimization)
- WebRTC & Real-time Communication
- Android / Web Application Development
- Cloud Infrastructure (AWS, Azure, Firebase)

## Publications

- "Practical Claude Code" — Context Engineering for Modern Development (Kindle EN/JP, Zenn)
- "LLMO — Three Paths for AI to Discover Your Content" (Kindle EN/JP, Zenn)
- "MCP Security in Practice" — Production Deployment Guide (Zenn, Free)
- "Claude Code QuickStart" (Zenn, Free)

## Links

- Website: https://propel-lab.co.jp/
- English Page: https://propel-lab.co.jp/en
- AI Information: https://propel-lab.co.jp/ai/
- LinkedIn: https://linkedin.com/in/kenimo49
- GitHub: https://github.com/kenimo49/propel-lab-website
